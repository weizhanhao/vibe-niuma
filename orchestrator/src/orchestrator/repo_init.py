...
任务：扫描仓库当前代码（不要假设、不要瞎编），生成 `AGENTS.md` 写到**项目根目录**。

**多仓项目说明**：这个项目根下可能含 N 个独立 git 子仓（典型：`frontend/.git` +
`backend/.git`）。要扫所有子仓的代码，但 `AGENTS.md` **只写一份**，落在项目根
（不是任一子仓内部）。文档里区分清楚「前端在 frontend/」「后端在 backend/」即可。
