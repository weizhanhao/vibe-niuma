...
export class CaptureOverlay {
  ...
  start(): Promise<CaptureResult | null> {
    ...
    this.root.setAttribute('data-vibe-niuma-overlay', '1');
  ...
  private onMouseDown = (e: MouseEvent) => {
    ...
    console.log('[vibe-niuma overlay] mousedown', e.clientX, e.clientY);
  };
  ...
  private onMouseUp = (e: MouseEvent) => {
    console.log('[vibe-niuma overlay] mouseup', e.clientX, e.clientY, 'dragStart=', this.dragStart);
    if (!this.dragStart || !this.box) return;
    ...
    this.dragStart = null;
    this.dragEnd = null;
    console.log('[vibe-niuma overlay] computed wxh=', w, h);
    if (w < 6 || h < 6) {
      this.box.style.display = 'none';
      console.log('[vibe-niuma overlay] BELOW 6px threshold, ignored');
      return;
    }
    this.finish({
