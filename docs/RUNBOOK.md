...
ECS 上 vibe-niuma 各组件的日常运维指引。

## 看日志

```bash
sudo journalctl -u vibe-niuma-orchestrator -n 200 -f
sudo journalctl -u vibe-niuma-llm-proxy    -n 200 -f
docker logs --tail 200 -f vibe-niuma-mysql
docker logs --tail 200 $(docker ps --filter name=vibe-niuma-preview- -q | head -1)
```

## 重启

```bash
sudo systemctl restart vibe-niuma-orchestrator
sudo systemctl restart vibe-niuma-llm-proxy
docker compose -f /opt/vibe-niuma/mysql/docker-compose.yml restart
```

## 改配置后生效

`/opt/vibe-niuma/.env` 改完：

```bash
sudo systemctl restart vibe-niuma-orchestrator vibe-niuma-llm-proxy
```

LiteLLM 配置 `/opt/vibe-niuma/llm-proxy/config.yml` 改完：
...
docker ps -a --filter name=vibe-niuma-preview- -q | xargs -r docker rm -f

# 顺便清镜像
docker image ls --filter reference='vibe-niuma-preview-*' -q | xargs -r docker rmi -f
```
...
| 现象 | 排查 |
|---|---|
| Orchestrator `journalctl` 反复 502/connection refused 给 ANTHROPIC_BASE_URL | llm-proxy 没起 / config 错。`systemctl status vibe-niuma-llm-proxy` |
...
| MySQL 连不上 | `docker logs vibe-niuma-mysql`；端口被本机占了？改 `MYSQL_PORT` |
| 业务员合并失败 `failed(merging, conflict)` | 期间 main 已变。让业务员丢弃后重新发起即可（设计 §5.5）|

## Plan 6 自助配置后的运维

Plan 6 把模型 / API key / 项目路径等配置从 `/opt/vibe-niuma/.env`（部署机改 + ssh + 重启）搬到了
`system_config` 表 + 扩展端引导向导。日常改配置不再需要 ssh ECS。

### admin.token —— 扩展与 admin API 之间的握手

`/opt/vibe-niuma/admin.token` 是 32 字节随机串，由 `vibe-niuma-orchestrator.service` 的 `ExecStartPre`
首次启动时生成，权限 `0600`，属主 root。

- **谁能读**：ECS 上能 ssh 进去的人（运维 + 部署机持有人）
- **怎么拿**：`ssh root@ECS 'cat /opt/vibe-niuma/admin.token'`，粘到扩展引导向导 Step 2
- **丢了 / 想换**：

  ```bash
  ssh root@ECS '
    rm -f /opt/vibe-niuma/admin.token
    systemctl restart vibe-niuma-orchestrator
    sleep 2
    cat /opt/vibe-niuma/admin.token
  '
  ```
...
```bash
ssh root@ECS "docker exec vibe-niuma-mysql mysql -uroot -pdemopass \
...
| 改的字段 | 期望发生 | 怎么验证 |
|---|---|---|
| `deepseek_api_key` / `dashscope_api_key` / `anthropic_api_key` | `vibe-niuma-llm-proxy.service` 自动重启（PUT 响应里 `restartedServices` 含 `litellm`） | `systemctl status vibe-niuma-llm-proxy \| head` 看 `Active:` 时间戳变了 |
...
- chrome.storage.local 的键：`vibe_niuma_config_v2`（zod schema 在 `extension/src/lib/config.ts`）
- 字段：`orchestratorUrl + adminToken + configVersion + server.{...}`
- API key 字段 **不存** chrome.storage，只在 PUT body 里传一次给 orchestrator

### 重置扩展全部配置（业务员侧）

`chrome://extensions/` → vibe-niuma 详情 → 「网站数据」/「清除存储」→ 重新装一遍引导向导。或在
扩展 service worker 的 DevTools console：

```js
chrome.storage.local.remove('vibe_niuma_config_v2')
```
...
3. orchestrator 启动失败 `/admin.token` 没生成 → `journalctl -u vibe-niuma-orchestrator -n 50`
